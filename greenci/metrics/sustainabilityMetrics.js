...
function getWorkerEfficiency() {
  const total = metrics.workerBusyTime + metrics.workerIdleTime

  if (total === 0) {
    return 0
  }

  return ((metrics.workerBusyTime / total) * 100).toFixed(1)
}
...
function getEnergyScore() {
  const score = 100 - (metrics.skippedStages * 0.5) + (metrics.pipelinesAvoided * 2)
...
function getMetrics() {
  return {
    pipelinesAvoided: metrics.pipelinesAvoided,
    skippedStages: metrics.skippedStages,
    computeSaved: metrics.computeSaved,
    workerEfficiency: getWorkerEfficiency(),
    queueEfficiency: getQueueEfficiency(),
